...
static void unpack_bulk_info(const struct bulk_info *info, uint64_t *addr,
                             uint32_t *rkey, uint64_t *len) {
  if (addr) *addr = ntohll_u64(info->addr);
  if (rkey) *rkey = ntohl(info->rkey);
  if (len) *len = ntohll_u64(info->len);
}
...
int main(int argc, char **argv) {
  ...
  if (total > remote_len) {
    fprintf(stderr, "Requested %" PRIu64 " bytes, remote has %" PRIu64
                    "; capping\n",
            total, remote_len);
    total = remote_len;
  }

  alloc_and_reg(&c, &c.buf_tx, &c.mr_tx, (size_t)chunk,
