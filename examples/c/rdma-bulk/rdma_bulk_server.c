...
static struct bulk_info pack_bulk_info(uint64_t addr, uint32_t rkey, uint64_t len)
{
    struct bulk_info info = {.addr = htonll_u64(addr), .rkey = htonl(rkey), .len = htonll_u64(len)};
    return info;
}
...
int main(int argc, char **argv)
{
    ...
    if (alloc_and_reg(&c, &c.buf_remote, &c.mr_remote, total,
    ...
    struct bulk_info info = pack_bulk_info((uintptr_t)c.buf_remote, c.mr_remote->rkey, total);
