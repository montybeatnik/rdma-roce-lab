...
#define CHECK(x, msg) do { if ((x)) { perror(msg); LOG("FAIL at %s:%d", __FILE__, __LINE__); _exit(1);} } while (0)
...
static inline void dump_qp(const struct ibv_qp *qp) {
    ...
    int mask = IBV_QP_STATE | IBV_QP_PATH_MTU | IBV_QP_DEST_QPN | IBV_QP_RQ_PSN |
               ...
               IBV_QP_SQ_PSN | IBV_QP_TIMEOUT | IBV_QP_RETRY_CNT |
               IBV_QP_RNR_RETRY | IBV_QP_MAX_QP_RD_ATOMIC;
    if (ibv_query_qp((struct ibv_qp*)qp, &attr, mask, &init) == 0) {
        LOG("QP dump: qpn=%u state=%s mtu=%d sq_psn=%u rq_psn=%u max_rd_atomic=%u max_dest_rd_atomic=%u retry=%u rnr_retry=%u timeout=%u",
            qp->qp_num, qp_state_str(attr.qp_state), attr.path_mtu, attr.sq_psn, attr.rq_psn,
            attr.max_rd_atomic, attr.max_dest_rd_atomic, attr.retry_cnt, attr.rnr_retry, attr.timeout);
        LOG("QP caps: max_send_wr=%u max_recv_wr=%u max_sge={s=%u,r=%u}",
            init.cap.max_send_wr, init.cap.max_recv_wr, init.cap.max_send_sge, init.cap.max_recv_sge);
    } else {
...
static inline const char* wc_opcode_str(enum ibv_wc_opcode op) {
    switch (op) {
        ...
        case IBV_WC_RECV_RDMA_WITH_IMM: return "RECV_RDMA_IMM";
        default: return "?";
    }
}
...
static inline void dump_wc(const struct ibv_wc *wc) {
    LOG("WC: wr_id=%lu status=%d opcode=%s byte_len=%u qp_num=%u",
        (unsigned long)wc->wr_id, wc->status, wc_opcode_str(wc->opcode), wc->byte_len, wc->qp_num);
}

static inline void dump_sge(const struct ibv_sge *s, const char *who) {
    LOG("SGE(%s): addr=%#llx len=%u lkey=0x%x",
        who, (unsigned long long)s->addr, s->length, s->lkey);
}
...
static inline void dump_wr_rdma(const struct ibv_send_wr *wr) {
    ...
    LOG("WR: wr_id=%lu opcode=%s signaled=%d num_sge=%d remote_addr=%#llx rkey=0x%x",
        (unsigned long)wr->wr_id, op,
        !!(wr->send_flags & IBV_SEND_SIGNALED), wr->num_sge,
