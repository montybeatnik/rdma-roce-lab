...
int build_pd_cq_qp(rdma_ctx *c, enum ibv_qp_type qpt, int cq_depth,
  ...
  c->pd = ibv_alloc_pd(c->id->verbs); CHECK(!c->pd, "ibv_alloc_pd");
  c->cq = ibv_create_cq(c->id->verbs, cq_depth, NULL, NULL, 0); CHECK(!c->cq, "ibv_create_cq");
  struct ibv_qp_init_attr qa = {
    .send_cq=c->cq, .recv_cq=c->cq,
    .cap = { .max_send_wr=max_send_wr, .max_recv_wr=max_recv_wr,
