...
int cm_create_channel_and_id(rdma_ctx *c) {
  c->ec = rdma_create_event_channel(); CHECK(!c->ec, "rdma_create_event_channel");
...
int cm_wait_event(rdma_ctx *c, enum rdma_cm_event_type want, struct rdma_cm_event **out) {
  ...
  if (ev->event != want) {
    LOG("Unexpected CM event: got=%d want=%d", ev->event, want);
...
int cm_server_accept_with_priv(rdma_ctx *c, const void *priv, size_t len) {
  struct rdma_conn_param p = {
    .private_data = priv, .private_data_len = (uint8_t)len,
...
int cm_client_connect(rdma_ctx *c, const char *ip, const char *port) {
  ...
  CHECK(cm_wait_event(c, RDMA_CM_EVENT_ADDR_RESOLVED, &ev), "wait ADDR_RESOLVED"); rdma_ack_cm_event(ev);
  CHECK(rdma_resolve_route(c->id, 2000), "rdma_resolve_route");
  CHECK(cm_wait_event(c, RDMA_CM_EVENT_ROUTE_RESOLVED, &ev), "wait ROUTE_RESOLVED"); rdma_ack_cm_event(ev);

  struct rdma_conn_param p = { .initiator_depth=16, .responder_resources=16, .retry_count=7, .rnr_retry_count=7 };
